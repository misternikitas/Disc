...
@bot.tree.command(name="translatehistory", description="Translate the latest messages in a channel to all linked channels")
@discord.app_commands.describe(channel="Source channel", limit="Number of latest messages to translate (default 100)")
async def translate_history(interaction: discord.Interaction, channel: discord.TextChannel, limit: int = 100):
    ...
    messages = [msg async for msg in channel.history(limit=limit, oldest_first=False) if not msg.author.bot and not msg.webhook_id]
    # Reverse to translate oldest → newest among the latest messages
    messages.reverse()
    ...
    await interaction.followup.send(f"✅ Translated the latest {count} messages from {channel.mention}", ephemeral=True)
